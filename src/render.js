...
function renderTextAnnotationPage(baseURI, canvasID, surface, apIndex) {
  ...
  if (Object.keys(xmls).length === 0 && Object.keys(htmls).length === 0)
    return null
...
function parseSurfaces(doc, teiDocumentID) {
  ...
  for (const surfaceEl of surfaceEls) {
    ...
    const width = Number.parseInt(surfaceEl.getAttribute('lrx'))
    const height = Number.parseInt(surfaceEl.getAttribute('lry'))
